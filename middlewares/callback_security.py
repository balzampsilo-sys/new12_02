    ...
    async def __call__(
        ...
        if message and message.date:
            ...
            if message_age > max_age:
                logging.info(
                    f"Blocking old callback from user {user_id}, " f"message age: {message_age}"
                )
        ...
        if self.enable_version_check and callback_data and ":" in callback_data:
            ...
            if parsed is None:
                # Устаревшая версия
                logging.warning(f"Outdated callback from user {user_id}: {callback_data}")
                ...
                await event.answer(
                    "⚠️ Устаревшая кнопка\n" "Используйте меню для нового действия", show_alert=True
                )
                return
        ...
        if self.enable_rate_limit:
            ...
            self.user_callbacks[user_id] = [
                t for t in self.user_callbacks[user_id] if t > cutoff_time
            ]
        ...
        try:
            ...
            logging.error(f"Error in callback handler for user {user_id}: {e}", exc_info=True)
        ...
        if state:
            ...
            if expected_hash:
                is_valid = await CallbackValidator.validate_state_integrity(
                    state_data, expected_hash
                )

                if not is_valid:
                    logging.warning(
                        f"State integrity check failed for user " f"{event.from_user.id}"
                    )

                    await state.clear()
                    await event.answer("❌ Данные устарели\n" "Начните заново", show_alert=True)
                    return
