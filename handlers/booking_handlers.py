...
async def select_day(callback: CallbackQuery, state: FSMContext):
    ...
    # ✅ ИСПРАВЛЕНО: Получаем динамические рабочие часы!
...
async def confirm_time(callback: CallbackQuery, state: FSMContext):
    ...
    # ✅ ИСПРАВЛЕНО: Получаем динамические рабочие часы для валидации!
    start_hour, end_hour = await SettingsRepository.get_work_hours()

    # ✅ ИСПРАВЛЕНО: Проверяем рабочие часы с динамическими значениями!
    if not validate_work_hours(time_obj.hour, start_hour, end_hour):
        await callback.answer(
            f"❌ Время вне рабочих часов ({start_hour}:00-{end_hour}:00)", show_alert=True
        )
        await state.clear()
        return
