...
from config import CANCELLATION_HOURS, DAY_NAMES, MAX_BOOKINGS_PER_USER
...
async def cancel_booking_improved(callback: CallbackQuery, state: FSMContext):
    ...
    # ✅ КРИТИЧНО: Валидация
    valid, data, error = await validate_booking_action(callback.data, callback.from_user.id)
    ...
    confirm_kb = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="❌ Нет, оставить", callback_data="v2:cancel_decline")],
        ]
    )
