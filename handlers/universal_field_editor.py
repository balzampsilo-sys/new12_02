...
class UniversalFieldEditor:
    ...
    @staticmethod
    async def get_records(config: FieldConfig, limit: int = 50) -> List[Tuple]:
        """
    ...
    @staticmethod
    async def get_record_by_id(config: FieldConfig, record_id: Any) -> Optional[Tuple]:
    ...
    @staticmethod
    async def update_field(config: FieldConfig, record_id: Any, new_value: str) -> bool:
        """Обновить поле"""
        query = f"""
            UPDATE {config.table}
    ...
    @staticmethod
    def validate_value(value: str, config: FieldConfig) -> Tuple[bool, Optional[str]]:
        """
...
async def field_editor_menu(message: Message):
    ...
    keyboard.append([InlineKeyboardButton(text="🔙 Назад", callback_data="v2:back_to_admin")])
...
async def select_field_type(callback: CallbackQuery, state: FSMContext):
    ...
    if not records:
        await callback.answer(f"ℹ️ Нет записей в {config.display_name}", show_alert=True)
        return
    ...
    keyboard.append(
        [InlineKeyboardButton(text="🔙 Назад", callback_data="v2:back_to_field_editor")]
    )
...
async def start_rename(callback: CallbackQuery, state: FSMContext):
    ...
    # Сохраняем в state
    await state.update_data(field_type=field_type, record_id=record_id, current_value=current_value)
...
async def apply_rename(message: Message, state: FSMContext):
    ...
    # Обновляем
    success = await UniversalFieldEditor.update_field(config, record_id, new_value)
