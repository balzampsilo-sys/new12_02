...
from config import CALLBACK_MESSAGE_TTL_HOURS, CALLBACK_VERSION, TIMEZONE
...
class CallbackValidator:
    ...
    async def validate_callback(
        ...
        if expected_parts:
            ...
            if actual_parts != expected_parts:
                ...
                return ValidationResult(
                    is_valid=False, error_message="❌ Ошибка данных кнопки", should_clear_state=True
                )
    ...
    async def validate_booking_callback(
        cls, callback: CallbackQuery, state: FSMContext, booking_id: int
    ) -> ValidationResult:
        """
    ...
    async def validate_service_callback(
        cls, callback: CallbackQuery, state: FSMContext, service_id: int
    ) -> ValidationResult:
        """
        ...
        if not service:
            return ValidationResult(
                is_valid=False, error_message="❌ Услуга не найдена", should_clear_state=True
            )
        ...
        return ValidationResult(is_valid=True, data={"service": service, "service_id": service_id})

    @classmethod
    async def validate_slot_callback(
        cls, callback: CallbackQuery, state: FSMContext, date_str: str, time_str: str
    ) -> ValidationResult:
        """
        ...
        return ValidationResult(
            is_valid=True, data={"date": date_str, "time": time_str, "datetime": slot_datetime}
        )

    @classmethod
    async def handle_validation_error(
        cls, callback: CallbackQuery, state: FSMContext, result: ValidationResult
    ):
        """
