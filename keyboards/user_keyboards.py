...
async def create_time_slots(
    ...
    # ✅ ИСПРАВЛЕНО: Получаем динамические рабочие часы из БД!
    ...
    # ✅ ИСПРАВЛЕНО: Проверяем слоты с учетом длительности услуги и динамических часов
    for hour in range(start_hour, end_hour):  # ✅ ИСПРАВЛЕНО: Динамические границы!
        ...
        # ✅ ИСПРАВЛЕНО: Проверяем что слот не выходит за ДИНАМИЧЕСКИЕ рабочие часы
        end_slot_hour = end_datetime.hour + (1 if end_datetime.minute > 0 else 0)
        if end_slot_hour > end_hour:  # ✅ ИСПРАВЛЕНО: Динамическая проверка!
            continue
